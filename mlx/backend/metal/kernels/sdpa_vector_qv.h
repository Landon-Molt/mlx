...
[[kernel]] void sdpa_vector_qv(
  ...
  for (int i = simd_gid; i < N; i += BN) {
    ...
    // 4-bit: 8 elements per uint32, extract 4-bit nibble
...
// 8-bit variant: same as above but V elements are packed 4 per uint32 (8 bits each)
template <typename T, int D>
[[kernel]] void sdpa_vector_qv8(
    const device T* queries [[buffer(0)]],
    const device T* keys [[buffer(1)]],
    const device uint* qv_data [[buffer(2)]],
    const device float* qv_scales [[buffer(3)]],
    ...
    const constant size_t& k_head_stride [[buffer(8)]],
    const constant size_t& k_seq_stride [[buffer(9)]],
    const constant size_t& qv_data_head_stride [[buffer(10)]],
    const constant size_t& qv_data_seq_stride [[buffer(11)]],
    const constant size_t& qv_group_head_stride [[buffer(12)]],
    const constant size_t& qv_group_seq_stride [[buffer(13)]],
    const constant float& scale [[buffer(14)]],
    uint3 tid [[threadgroup_position_in_grid]],
    uint3 tpg [[threadgroups_per_grid]],
    uint simd_gid [[simdgroup_index_in_threadgroup]],
  ...
  // Identical to sdpa_vector_qv except V dequant uses 8-bit (4 per uint32)
  constexpr int BN = 32;
  constexpr int BD = 32;
  constexpr int qk_per_thread = D / BD;
  constexpr int v_per_thread = D / BD;
  constexpr int n_groups = D / 64;  // 8-bit uses group_size=64
  ...
  for (int i = simd_gid; i < N; i += BN) {
    ...
    // 8-bit dequant: 4 elements per uint32, extract 8-bit byte
    for (int j = 0; j < v_per_thread; j++) {
      int elem = int(simd_lid) * v_per_thread + j;
      int word_idx = elem / 4;        // 4 elements per uint32
      int byte_pos = (elem % 4) * 8;  // 8 bits per element
      uint raw = (vd_ptr[word_idx] >> byte_pos) & 0xFFu;

      int grp = elem / 64;  // group_size=64 for 8-bit
      U v_val = U(raw) * vs_ptr[grp] + vb_ptr[grp];
