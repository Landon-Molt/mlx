...
[[kernel, max_total_threads_per_threadgroup(WM * WN * 32)]] void attention_qv(
  ...
  constexpr int packed_per_row = BD / 8;  // uint32 words per V row (4-bit)
  ...
  // Standard Q and K loaders (fp16)
  using QBlockLoader = BlockLoaderT<T, BQ, BD, LDQ_tgp, 1, 1, WM * WN * 32>;
  using KBlockLoader = BlockLoaderT<T, BK, BD, LDK_tgp, 1, 0, WM * WN * 32>;
  ...
  for (int kb = 0; kb < kb_lim; kb++) {
    ...
    // Load V tile: dequant mx.quantize scalar → fp16 in threadgroup memory
    {
      ...
      for (int e = 0; e < elems_per_thread; e++) {
        ...
        if (t_local < valid_tokens && t_global < params->kL) {
          // Scalar dequant: int4 * scale + bias (2 ALU ops)
          const device uint* vd_row = vd_base + t_global * packed_per_row;
          int word_idx = d / 8;
          int bit_pos = (d % 8) * 4;
          uint raw = (vd_row[word_idx] >> bit_pos) & 0xFu;

          int grp = d / qv_group_size;
          int grp_offset = t_global * grp_per_row + grp;
          float v_val = float(raw) * vs_base[grp_offset] + vb_base[grp_offset];
          val = T(v_val);
        }
